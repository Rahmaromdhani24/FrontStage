...
@Component({
  selector: 'app-personnel-mensuel',
  templateUrl: './personnel-mensuel.component.html',
  styleUrls: ['./personnel-mensuel.component.css']
})
export class PersonnelMensuelComponent implements OnInit ,AfterViewInit  {
  showLoader : boolean = true; 
  informationsPersonnel: any // Personnel | null = null; 

  navText = 'Personnel Mensuel';
  ...
  getAllPersonnels() {
    this.service.getAllPersonnelsMensuel().subscribe((data: Personnel[]) => {
    this.service.PersonnelsData = data.sort((a, b) => a.mle - b.mle);
    $(document).ready(function () {
    $('#homePersonnels').DataTable();   });
    });  
  }
  ...
  afficheInformations(mle :string){
    this.service.getPersonnelMensuel(mle).subscribe(data => {
      this.informationsPersonnel = data;
      console.log("matricule personnel "+mle)
