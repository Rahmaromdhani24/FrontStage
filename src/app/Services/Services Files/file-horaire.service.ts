...
export class FileHoraireService {
  dateSelected : any ; 
  constructor(private http: HttpClient) {
    this.dateSelected = localStorage.getItem('DateSelected')}

  private apiUrl = 'http://localhost:8281/api/historiques/downloadExcelHoraire'; 
  generateExcelHoraire(dateEffet: string): void {
    ...
    this.http.get(this.apiUrl, { params, responseType: 'blob' }).subscribe(blob => {
      saveAs(blob, 'avancement_horaire'+dateEffet+'.xlsx');
    }, error => {
      console.error('Error downloading the file', error);
    });
  }

  private urlGenerateExcel57ans = 'http://localhost:8281/api/historiques/downloadHoraire57ans'; 
        ...
        for (let row of result) {
          ...
          try {
            const item = await new Promise<any>((resolve, reject) => {
              this.getChangementHoraire(row.mle,  this.dateSelected).subscribe(
                item => resolve(item),
                error => reject(error)
              );
            });
...
doc.line(lineStartX2, 26, lineEndX2, 26); // Ligne juste en dessous du texte décalée vers la droite
 ...
 doc.save('etatHoraire_'+ this.dateSelected+'.pdf');
},
error => {
