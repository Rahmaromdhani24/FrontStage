...
export class FileMensuleService {
  dateSelected : any ; 
  constructor(private http: HttpClient) {
    this.dateSelected = localStorage.getItem('DateSelected')}

private apiUrl = 'http://localhost:8281/api/historiques/downloadExcelMensuel'; // URL de votre API backend  
generateExcelMensuel(dateEffet: string): void {
  dateEffet = this.dateSelected  ; 
  const params = new HttpParams().set('dateEffet',  dateEffet );
  this.http.get(this.apiUrl, { params, responseType: 'blob' }).subscribe(blob => {
    saveAs(blob, 'avancement.xlsx');
  }, error => {
    console.error('Error downloading the file', error);
  });
}
private urlGenerateExcel57ans = 'http://localhost:8281/api/historiques/downloadMensule57ans'; 
generateExcelMensuel57ans(dateEffet: string): void {
  dateEffet =  this.dateSelected  ; 
  const params = new HttpParams().set('dateEffet', dateEffet);
  this.http.get(this.urlGenerateExcel57ans, { params, responseType: 'blob' }).subscribe(blob => {
...
downloadPDFMensuel(): void {
  this.getData( this.dateSelected).subscribe(
    async (result: any[]) => {
      ...
      for (let row of result) {
        ...
        try {
          const item = await new Promise<any>((resolve, reject) => {
            this.getChangementMensuel(row.mle,  this.dateSelected).subscribe(
              item => resolve(item),
              error => reject(error)
            );
          });
          if (item) {
            ...
            const indDifferentiel = item.indDiff !== null ? item.indDiff : '-';
            ...
            data.push([            
                ...
                { content: row.indDiffA, styles: { halign: 'center', valign: 'middle' } }, // Cellule vide pour `sbasee
      ...
      const text1 = 'ETAT D\'AVANCEMENT D\'ECHELON NORMAL - '+this.transformDateToMonthYear( this.dateSelected);
      ...
      doc.save('etatMensuel_'+ this.dateSelected+'.pdf');
     },
     error => {
    ...
    transformDateToMonthYear(dateString: string): string {
      ...
      const monthIndex = +dateParts[1] - 1; // Convertit en index de mois (0 = janvier) 
